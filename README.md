...
- 核验常因浏览器误刷新中断。每次基准或复检分析成功后，页面把**原始 PNG 字节、
  当前对比阶段（待复检 / 已完成对比）与选中方位**写入带结构版本号的
  IndexedDB 快照（`src/lib/sessionSnapshot.ts`，版本常量 `SNAPSHOT_VERSION`）。
  快照只保存原始输入与会话位置，**不保存命中数、判定等旧结果**；照明校准状态
  也不纳入快照。
- 页面重新加载时读取快照，恢复期间核验页显示「正在恢复」。恢复不直接信任旧判定，
  而是把保存的原始 PNG 当作全新上传，**仍通过现有 PNG 签名校验、`createImageBitmap`
  原生解码、尺寸校验与 Canvas 取样**，再由 `analyzePixels` / `comparePixels`
  重建 Analysis、ZoneResult 与 32×32 差异图。恢复成功后回到刷新前的位置：
  待复检阶段的单图缺口审阅，或已完成对比的前后对比与差异图，选中方位一并恢复。
- 快照缺图、结构版本不识别、PNG 损坏（任一流水线阶段失败）或浏览器存储不可用时，
  页面明确说明无法恢复并**清理该快照**，随后允许正常上传；首次访问（无快照）与
  禁用存储时不闪现恢复提示，既有核验流程完全不受阻。
- 操作员可随时点「清除已保存会话」删除快照并回到初始上传态；清除后再次刷新
  不会恢复出旧结果。重新上传基准（即使随后上传失败）也会先删除旧快照，
  成功后立即写入新快照；取消对比会把快照回退到「仅基准、待复检」阶段。
...
npm run e2e                           # Playwright：角标核验全部验收 + 复检对比（固定不合格基准、上传改善图、32×32 差异图、失败阶段与保留基准、取消对比）+ 会话恢复（819 不合格基准刷新恢复选中方位、满命中复检刷新核对四类像素计数、损坏/不识别快照降级、主动清除后不复活、禁用存储不受阻）+ 照明校准（通过报告、暗角热力图与结论、错误阶段提示、失败后重试、返回核验页状态保持）
...
src/lib/sessionSnapshot.ts   误刷新恢复：带结构版本的 IndexedDB 快照读写/清理与结构校验（只存原始 PNG、对比阶段、选中方位）
